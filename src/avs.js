...
CodeMirror.defineMode("avs", function (config, parserConfig) {
  ...
  let constants = parserConfig.constants || {};
  let variables = parserConfig.variables || {};
  ...
  function tokenBase(stream, state) {
    ...
    if (contains(variables, cur)) {
      return "variable";
    }
    if (contains(constants, cur)) {
      return "variable-2";
    }
...
let builtin = words("abs sin cos tan asin acos atan atan2 sqr sqrt invsqrt pow exp log log10 floor ceil sign min max sigmoid rand band bor bnot if assign exec2 equal above below getosc getspec gettime getkbmouse megabuf gmegabuf loop");
let constants = words("$e $phi $pi $E $PHI $PI");
let indentSwitch = false;

def(["avs/bump"], {
  name: "avs",
  variables: words("x y isBeat isLBeat bi"),
  builtin: builtin,
  constants: constants,
...
def(["avs/color-modifier", "avs/cm"], {
  name: "avs",
  variables: words("red green blue"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/dynamic-movement", "avs/dm"], {
  name: "avs",
  variables: words("x y w h r d alpha"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/dynamic-shift", "avs/ds"], {
  name: "avs",
  variables: words("x y w h b alpha"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/effect-list", "avs/el"], {
  name: "avs",
  variables: words("enabled beat clear alphain alphaout w h"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/superscope", "avs/ssc"], {
  name: "avs",
  variables: words("n x y i v b red green blue linesize skip drawmode w h"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/texer2", "avs/t2"], {
  name: "avs",
  variables: words("n w h i x y v b iw ih sizex sizey red green blue skip"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["avs/triangle", "avs/tr"], {
  name: "avs",
  variables: words("n x1 x2 x3 y1 y2 y3 z1 skip red1 blue1 green1 w h zbclear zbuf"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
...
def(["text/x-avs", "avs"], {
  name: "avs",
  variables: words("alpha alphain alphaout b beat bi blue blue1 clear d drawmode enabled green green1 h i ih isBeat isLBeat iw linesize n r red red1 sh sizex sizey skip sw v w x x1 x2 x3 y y1 y2 y3 z1 zbclear zbuf"),
  builtin: builtin,
  constants: constants,
  indentSwitch: indentSwitch,
});
